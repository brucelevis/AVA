...
void editor_draw() {
    ...
    float kw = !!(io.KeysDown[87/*GLFW_KEY_W*/] | io.KeysDown[265/*GLFW_KEY_UP*/]);
    float ka = !!(io.KeysDown[65/*GLFW_KEY_A*/] | io.KeysDown[263/*GLFW_KEY_LEFT*/]);
    float ks = !!(io.KeysDown[83/*GLFW_KEY_S*/] | io.KeysDown[264/*GLFW_KEY_DOWN*/]);
    float kd = !!(io.KeysDown[68/*GLFW_KEY_D*/] | io.KeysDown[262/*GLFW_KEY_RIGHT*/]);
    float ke = !!(io.KeysDown[69/*GLFW_KEY_E*/] | io.KeysDown[81/*GLFW_KEY_Q*/]);
    float kc = !!(io.KeysDown[67/*GLFW_KEY_C*/]);
    ...
    if( 1 ) { // if cam active
        static float cam_pos[3] = {10,10,10};
        static float cam_quat[4] = {0.319360, 0.288000, 0.896952, -0.102543}; //quat_norm(cam_quat, cam_quat);
        ...
        flycamera( cam_pos, cam_quat, cam_mat3, look_mult, move_mult * delta, mouse_diff.x, mouse_diff.y, kw, ka, ks, kd, ke, kc );

        mat4x4 r, s;
        mat4x4_identity(r);
        mat4x4_from_quat(r, cam_quat);
        mat4x4_scale_aniso(r, r, -cam_scale[0], cam_scale[1], -cam_scale[2]);
        mat4_invert(s, r); // inverted!
        mat4x4_translate_in_place(s, cam_pos[0], cam_pos[1], cam_pos[2]);

        memcpy(c->transform, &s[0][0], 16 * sizeof(float));

        //printf("%f %f %f (%f %f) (%f %f %f %f)\n", cam_pos[0], cam_pos[1], cam_pos[2], c->XAngle, c->YAngle, cam_quat[0], cam_quat[1], cam_quat[2], cam_quat[3]);
     }
#endif
